...
def write_voxelyze_file(sim, env, generations, individual_id, morphogens, fitness, im_size, run_directory, run_name):
    ...
    voxelyze_file.write(
        ...
        <X_Voxels>" + str(im_size) + "</X_Voxels>\n\
        <Y_Voxels>" + str(im_size) + "</Y_Voxels>\n\
        <Z_Voxels>" + str(1) + "</Z_Voxels>\n")
    ...
    for z in range(1):
        voxelyze_file.write("<Layer><![CDATA[")
        for y in range(im_size):#reversed(range(im_size)):
            for x in reversed(range(im_size)):#range(im_size):
                state = int(morphogens[0][y][x])
                voxelyze_file.write(str(state))
                #voxelyze_file.write(", ")
        voxelyze_file.write("]]></Layer>\n")
    voxelyze_file.write("</Data>\n")
...
def write_voxelyze_file_vis(sim, env, generations, individual_id, morphogens, fitness, im_size, run_directory, run_name):

    # TODO: work in base.py to remove redundant static text in this function
  
    voxelyze_file = open(run_directory + run_name + "--gene_{0}_id_{1}_fitness_{2}.vxa".format( generations, individual_id, fitness), "w")
    #voxelyze_file = open("test.vxa" , "w")
