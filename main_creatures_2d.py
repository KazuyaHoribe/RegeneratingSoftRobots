...
from GA_2d import GeneticAlgorithm
from genotype_net import MorphNet
from evaluation_2d import run_robot_simulation
...
def main(argv):
  ...
  #Creatuer settings
    parser.add_argument('--im_size', type=int, default=7, metavar='N',\
      help='Size of creature')
    parser.add_argument('--voxel_types', type=int, default=5, metavar='N',\
      help='How many different types of voxels')
    parser.add_argument('--number_neighbors', type=int, default=5, metavar='N',\
  ...
  # Voxelyze sim settings
    parser.add_argument('--simtime',type=int,default=1.25,\
      help='Simulation time in voxelyze')
    parser.add_argument('--initime',type=int,default=1,\
  ...
  #General settings
    ...
    parser.add_argument('--run_directory',type=str,default='2d_basic_data')
    ...
    'simtime':args.simtime, 'initime':args.initime, 'run_directory':args.run_directory, 'run_name':args.run_name,\
    'number_neighbors':args.number_neighbors, 'sigma':args.sigma, 'N':args.N, 'fig_output_rate':args.fig_output_rate}
    ...
    # Add sleep/nonsleep value to in/output
    ...
    weight_path = args.run_directory + "/weightFiles/"
    ...
    if args.optimizer=='ga':
      ga = GeneticAlgorithm(model.get_weights(), run_robot_simulation, population_size=args.popsize, sigma=0.03, \
      ...
      es = EvolutionStrategy(model.get_weights(), ca_fitness, population_size=args.popsize, sigma=0.03,\
